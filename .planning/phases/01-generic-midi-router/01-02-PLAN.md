...
<task type="auto">
  ...
  <files>rp2040/rp2040.ino, rp2040/serial_midi_handler.cpp</files>
  <action>
  ...
  **Update midi_router.h** to add `byte subType;` to MidiMessage struct (value 0 = on/default, 1 = off/alternate). Then update midi_router.cpp's forwardToInterface to check `msg.subType` for MIDI_MSG_NOTE to decide between sendNoteOn/sendNoteOff.
  ...
  - `usbh_onNoteOnHandle` → type=NOTE, subType=0
  - `usbh_onNoteOffHandle` → type=NOTE, subType=1
  - `usbh_onPolyphonicAftertouchHandle` → type=POLY_AFTERTOUCH
  - `usbh_onControlChangeHandle` → type=CONTROL_CHANGE
  - `usbh_onProgramChangeHandle` → type=PROGRAM_CHANGE
  - `usbh_onAftertouchHandle` → type=CHANNEL_AFTERTOUCH
  - `usbh_onPitchBendHandle` → type=PITCH_BEND, use msg.pitchBend
  - `usbh_onSysExHandle` → type=SYSEX, msg.sysexData=array, msg.sysexSize=size, msg.channel=0
  - `usbh_onMidiClockHandle` → type=REALTIME, msg.rtType=midi::Clock, msg.channel=0
  - `usbh_onMidiStartHandle` → type=REALTIME, msg.rtType=midi::Start, msg.channel=0
  - `usbh_onMidiContinueHandle` → type=REALTIME, msg.rtType=midi::Continue, msg.channel=0
  - `usbh_onMidiStopHandle` → type=REALTIME, msg.rtType=midi::Stop, msg.channel=0
  ...
  **In `serial_midi_handler.cpp`:**
  ...
  - `localSerialOnNoteOn` → type=NOTE, subType=0
  - `localSerialOnNoteOff` → type=NOTE, subType=1
  - `localSerialOnControlChange` → type=CONTROL_CHANGE
  - `localSerialOnProgramChange` → type=PROGRAM_CHANGE
  - `localSerialOnAftertouch` → type=CHANNEL_AFTERTOUCH
  - `localSerialOnPitchBend` → type=PITCH_BEND
  - `localSerialOnSysEx` → type=SYSEX
  - `localSerialOnClock` → type=REALTIME, rtType=midi::Clock
  - `localSerialOnStart` → type=REALTIME, rtType=midi::Start
  - `localSerialOnContinue` → type=REALTIME, rtType=midi::Continue
  - `localSerialOnStop` → type=REALTIME, rtType=midi::Stop
